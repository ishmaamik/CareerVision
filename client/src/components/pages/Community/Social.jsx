...
const Social = ({ searchTerm = "", category = "All" }) => {
  ...
  if (!user) {
    return (
      <div
        className={`min-h-screen ${themeClasses.bg.primary} flex items-center justify-center`}
      >
        <div
          className={`${componentStyles.card} p-8 text-center max-w-md mx-auto`}
        >
          <h2
  ...
  return (
    <div
      className={`min-h-screen ${themeClasses.bg.primary} transition-all duration-300`}
    >
      {/* Header */}
      <div
        className={`${themeClasses.bg.surface} shadow-lg sticky top-0 z-10 backdrop-blur-lg bg-opacity-95`}
      >
        <div className="max-w-4xl mx-auto px-4 py-4">
          <h1
            className={`text-3xl font-bold ${themeClasses.text.primary} bg-gradient-to-r ${themeClasses.brand.gradient} bg-clip-text text-transparent`}
          >
            Social Feed
          </h1>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <Fade in={true} timeout={800}>
          <div
            className={`${componentStyles.card} p-6 transform ${
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Tooltip title="Add Photo">
                      <label
                        className={`${componentStyles.button.secondary} cursor-pointer inline-flex items-center space-x-2 !py-2 !px-4`}
                      >
        ...
        {loading && (
          <div className={`${componentStyles.card} p-8 text-center`}>
            <div className="animate-pulse space-y-4">
              <div
                className={`h-4 ${themeClasses.bg.accent} rounded w-3/4 mx-auto`}
              ></div>
              <div
                className={`h-4 ${themeClasses.bg.accent} rounded w-1/2 mx-auto`}
              ></div>
            </div>
            <p className={`${themeClasses.text.secondary} mt-4`}>
              Loading your feed...
            </p>
          </div>
        )}
                ...
                >
                  ...
                  >
                      ...
                      <div className="flex items-center space-x-3">
                        <Avatar
                          ...
                          sx={{
                            ...
                            height: 45,
                            border: `2px solid ${
                              isDarkMode ? "#22C55E" : "#D4A574"
                            }`,
                          }}
                        />
                        <div>
                          <h3
                            className={`font-semibold ${themeClasses.text.primary} hover:${themeClasses.brand.primary} cursor-pointer transition-colors`}
                          >
                            {post.postedBy?.name || "Unknown User"}
                          </h3>
                          <div className="flex items-center space-x-2">
                            <span
                              className={`text-sm ${themeClasses.text.muted} flex items-center space-x-1`}
                            >
                              <FaClock size={12} />
                    ...
                    <div className="mb-4">
                      {post.post && (
                        <p
                          className={`${themeClasses.text.primary} text-base leading-relaxed whitespace-pre-wrap`}
                        >
                          {post.post}
                        </p>
                      )}
                    ...
                    {/* Post Actions */}
                    <div
                      className={`flex items-center justify-between pt-4 border-t ${themeClasses.border.divider}`}
                    >
            ...
            : !loading && (
                <Fade in={true} timeout={800}>
                  <div className={`${componentStyles.card} p-12 text-center`}>
                    <EmojiEmotions
                      className={`text-6xl ${themeClasses.text.muted} mb-4`}
                    />
                    <h3
